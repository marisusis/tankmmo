tg.TankGame = new Class({
	...
	construct: function(options) {
		...
		******************/
		// Sound
		this.sound = new tg.Sound({
			sounds: tg.config.sounds
		});
		...
		******************/
		...
		this.hook(this.tank.controlsLoopCb);
		...
		******************/
		...
		// Load a map
		this.loadMap(tg.maps['Lollypop Land']);
	...
	processEnemyBullets: function() {
		...
		removeBulletLoop: for (var i = 0; i < this.enemyBullets.length; i++) {
			...
			if (age > tg.config.weapons[bullet.type].time) {
			...
			for (var j = 0; j < this.mapItems.length; j++) {
				...
				if (this.isHit(itemModel, bulletModel, bullet.type)) {
					...
					{ // TEMPORARY REMOVE THIS
						mapItem.takeHit(tg.config.weapons[bullet.type].damage);
	...
	isHit: function(target, bullet, type) {
		...
		return !!(intersects.length && intersects[0].distance < tg.config.weapons[type].hitDistance);
	},
